...
export const usersLoaderQuery = `query All($block: Int! 
  ...
  positions(first: $first block: {number: $block} where: {id_gt: $lastPositionsId, user_in: $users, market_in: $markets} orderBy: id) {
    id
    user {id}
    market {
      id 
  ...
  metaMorphoPositions(first: $first block: {number: $block} where: {id_gt: $lastMetaMorphoPositionsId user_in: $users metaMorpho_in: $metaMorphos} orderBy: id) {
    id
    metaMorpho {
      id
      totalShares
...
export const loadFullForUsersAndMarketsAndMetaMorphos = async (
  ...
  const cacheKey = `${subgraph.url}-${block}-${JSON.stringify({ users, marketsFilter, metaMorphosFilter })}`;
